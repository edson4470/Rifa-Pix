...
export function PaginaCompra() {
  ...
  const handleFinalizar = async () => {
    ...
    try {
      // 1. BUSCA BILHETES REAIS DISPONÍVEIS NA TABELA 'bilhetes'
      ...
      // 2. REGISTRA A VENDA NA TABELA 'vendas'
      ...
      // 3. ATUALIZA O STATUS DOS BILHETES PARA 'reservado' (Evita duplicidade!)
      const { error: erroAtualizacao } = await supabase
        .from('bilhetes')
        .update({ status: 'reservado', comprador_nome: nome })
        .in('id', idsBilhetes);
      ...
      <div className="max-w-md mx-auto p-4 mt-2">
        ...
        {etapa === 5 && (
          <div className="bg-[#18181b] border border-[#27272a] rounded-xl p-6 text-center animate-in slide-in-from-right-8">
            ...
            <button 
              onClick={() => window.open(`https://wa.me/?text=Olá! Acabei de pagar a reserva da rifa ${campanha.nome}. Meu nome é ${nome} e meus números são: ${numerosSorte.join(', ')}. Segue meu comprovante!`, '_blank')} 
