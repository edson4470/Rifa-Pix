...
export function GerenciarCampanha() {
  ...
  // Estado que controla qual janela (modal) geral está aberta
  const [modalAtivo, setModalAtivo] = useState<string | null>(null);
  
  // Estados EXCLUSIVOS para a tela de "Minhas Vendas"
  const [vendasView, setVendasView] = useState<'lista' | 'relatorio'>('lista');
  const [mostrarFiltros, setMostrarFiltros] = useState(false);

  // ================= ESTADOS DOS FILTROS (A Mágica do Clique) =================
  const [filtroOrdem, setFiltroOrdem] = useState('recentes'); // Guarda a ordem escolhida
  const [filtroStatus, setFiltroStatus] = useState<string[]>([]); // Guarda as transações escolhidas

  // Função para marcar/desmarcar os tipos de transação
  const toggleStatus = (status: string) => {
    setFiltroStatus(prev => 
      prev.includes(status) 
        ? prev.filter(s => s !== status) // Se já tem, tira
        : [...prev, status] // Se não tem, coloca
    );
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 text-white animate-in fade-in duration-500">
      ...
      {/* ===================== ÁREA DE MODAIS (As janelas mágicas) ===================== */}
        ...
        <div className="fixed inset-0 bg-black/90 flex flex-col items-center justify-center p-4 md:p-10 z-[60] animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="bg-[#18181b] w-full max-w-5xl h-[90vh] rounded-xl border border-[#27272a] shadow-2xl flex flex-col relative overflow-hidden">
            ...
            <div className="flex items-center justify-between p-6 border-b border-[#27272a] shrink-0">
              <button 
                onClick={() => vendasView === 'relatorio' ? setVendasView('lista') : fecharModal()} 
              ...
              <h2 className="text-xl font-bold text-white">
                {vendasView === 'lista' ? 'Minhas vendas' : 'Relatório da campanha'}
              </h2>
                ...
                <div className="max-w-3xl mx-auto space-y-6">
                  ...
                  <div className="flex flex-col md:flex-row gap-3">
                    <div className="flex-1 flex bg-[#18181b] border border-[#27272a] rounded overflow-hidden">
                      <select className="bg-transparent text-zinc-400 px-4 py-3 border-r border-[#27272a] outline-none text-sm cursor-pointer hover:bg-[#27272a] transition-colors">
                        <option>Filtro</option>
                        <option>Nome</option>
                        <option>Telefone</option>
                        <option>Cota</option>
                      </select>
                      <input type="text" placeholder="Buscar" className="flex-1 bg-transparent px-4 outline-none text-white text-sm" />
                  ...
                  <h3 className="text-xl font-bold text-white mt-8">Resultados (0)</h3>

                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    <svg className="w-24 h-24 text-zinc-600 mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2zM10 8.5a.5.5 0 11-1 0 .5.5 0 011 0zm5 5a.5.5 0 11-1 0 .5.5 0 011 0z"></path>
                    </svg>
              ...
              {vendasView === 'relatorio' && (
                <div className="max-w-2xl mx-auto space-y-4">
                  <div className="bg-[#18181b] border border-blue-500/50 rounded-lg p-5 flex items-center gap-6 shadow-sm">
                    <div className="w-16 h-16 rounded bg-blue-500/10 flex items-center justify-center shrink-0">
                      <svg className="w-8 h-8 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                    </div>
                    <div>
                      <p className="text-zinc-400 text-sm mb-1">Visitas no site</p>
                      <p className="text-2xl font-bold text-white">0</p>
                    </div>
                  </div>

                  <div className="bg-[#18181b] border border-orange-400/50 rounded-lg p-5 flex items-center gap-6 shadow-sm">
                    <div className="w-16 h-16 rounded bg-orange-400/10 flex items-center justify-center shrink-0">
                      <svg className="w-8 h-8 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                    </div>
                    <div className="flex-1">
                      <p className="text-zinc-400 text-sm mb-2">Participantes únicos</p>
                      <div className="flex gap-4">
                        <p className="text-sm font-medium flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#22c55e]"></span> 0 Efetuou o pagamento</p>
                        <p className="text-sm font-medium flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"></span> 0 Reservou e não pagou</p>
                      </div>
                    </div>
                  </div>

                  <div className="bg-[#18181b] border border-[#22c55e]/50 rounded-lg p-5 flex items-center gap-6 shadow-sm">
                    <div className="w-16 h-16 rounded bg-[#22c55e]/10 flex items-center justify-center shrink-0">
                      <svg className="w-8 h-8 text-[#22c55e]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                    <div>
                      <p className="text-zinc-400 text-sm mb-1">Vendas realizadas</p>
                      <p className="text-xs text-zinc-500 mb-1">Pedidos</p>
                      <p className="text-2xl font-bold text-white">R$ 0,00</p>
                    </div>
                  </div>

                  <div className="bg-[#18181b] border border-yellow-500/50 rounded-lg p-5 flex items-center gap-6 shadow-sm">
                    <div className="w-16 h-16 rounded bg-yellow-500/10 flex items-center justify-center shrink-0">
                      <svg className="w-8 h-8 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                    <div>
                      <p className="text-zinc-400 text-sm mb-1">Reservas</p>
                      <p className="text-xs text-zinc-500 mb-1">Pedidos</p>
                      <p className="text-2xl font-bold text-white">R$ 0,00</p>
                    </div>
                  </div>

                  <div className="bg-[#18181b] border border-red-500/50 rounded-lg p-5 flex items-center gap-6 shadow-sm">
                    <div className="w-16 h-16 rounded bg-red-500/10 flex items-center justify-center shrink-0">
                      <svg className="w-8 h-8 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                    <div>
                      <p className="text-zinc-400 text-sm mb-1">Reservou e não pagou</p>
                      <p className="text-xs text-zinc-500 mb-1">Pedidos</p>
                      <p className="text-2xl font-bold text-white">R$ 0,00</p>
                    </div>
                  </div>

                  <button className="w-full mt-6 bg-[#18181b] border border-[#27272a] text-white font-bold py-4 rounded hover:border-[#22c55e] transition-colors">
                    Baixar relatório
                  </button>
          ...
          {/* OVERLAY DE FILTROS COM CAIXINHAS CLICÁVEIS */}
          {mostrarFiltros && (
            <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-[70] backdrop-blur-sm p-4">
              <div className="bg-[#18181b] w-full max-w-md rounded-xl border border-[#27272a] shadow-2xl p-6 relative animate-in zoom-in-95 duration-150">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button onClick={() => setMostrarFiltros(false)} className="text-orange-400 hover:text-white transition-colors">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                  </button>
                </div>

                <div className="space-y-6">
                  {/* ORDENAR POR (Apenas 1 por vez) */}
                  <div>
                    ...
                    <label className="flex items-center gap-3 mb-2 cursor-pointer group" onClick={() => setFiltroOrdem('recentes')}>
                      <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${filtroOrdem === 'recentes' ? 'border-[#22c55e] bg-[#22c55e]' : 'border-[#27272a] bg-[#09090b] group-hover:border-[#22c55e]'}`}>
                        {filtroOrdem === 'recentes' && <svg className="w-3.5 h-3.5 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>}
                      </div>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer group" onClick={() => setFiltroOrdem('antigos')}>
                      <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${filtroOrdem === 'antigos' ? 'border-[#22c55e] bg-[#22c55e]' : 'border-[#27272a] bg-[#09090b] group-hover:border-[#22c55e]'}`}>
                        {filtroOrdem === 'antigos' && <svg className="w-3.5 h-3.5 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>}
                      </div>
                      <span className={`transition-colors ${filtroOrdem === 'antigos' ? 'text-white' : 'text-zinc-400 group-hover:text-white'}`}>Mais antigo</span>
                    </label>
                  </div>

                  {/* TIPO DE TRANSAÇÃO (Pode marcar vários) */}
                  <div>
                    ...
                    {[
                      ...
                      return (
                        <label key={status.id} className="flex items-center gap-3 mb-2 cursor-pointer group" onClick={() => toggleStatus(status.id)}>
                          <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isChecked ? 'border-[#22c55e] bg-[#22c55e]' : 'border-[#27272a] bg-[#09090b] group-hover:border-[#22c55e]'}`}>
                            {isChecked && <svg className="w-3.5 h-3.5 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>}
                          </div>
      ...
      {/* OUTROS MODAIS (Apenas para não sumirem da tela) */}
      {modalAtivo === 'premiado' && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[60]">
          <div className="bg-[#18181b] p-6 rounded-xl w-full max-w-md border border-[#27272a] shadow-2xl relative">
            ...
            <input type="text" placeholder="Ex: 045" className="w-full bg-[#09090b] border border-[#27272a] rounded p-3 mb-4 focus:border-[#22c55e] outline-none text-white text-center text-xl tracking-widest" />
            <button className="w-full bg-[#22c55e] text-black font-bold py-3 rounded hover:bg-green-600 transition-colors">Verificar Ganhador</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[60]">
          <div className="bg-[#18181b] p-6 rounded-xl w-full max-w-sm border border-[#27272a] shadow-2xl relative">
            ...
            <div className="space-y-3">
              <div className="bg-[#09090b] border border-[#27272a] p-4 rounded flex items-center gap-4">
                <div className="w-8 h-8 rounded-full bg-yellow-500/20 text-yellow-500 flex items-center justify-center font-black border border-yellow-500/50">1</div>
                <div><p className="font-bold text-sm">Carlos Eduardo</p><p className="text-xs text-zinc-400">150 cotas</p></div>
              </div>
              <div className="bg-[#09090b] border border-[#27272a] p-4 rounded flex items-center gap-4">
                <div className="w-8 h-8 rounded-full bg-zinc-400/20 text-zinc-300 flex items-center justify-center font-black border border-zinc-400/50">2</div>
                <div><p className="font-bold text-sm">Mariana Souza</p><p className="text-xs text-zinc-400">85 cotas</p></div>
              </div>
              <div className="bg-[#09090b] border border-[#27272a] p-4 rounded flex items-center gap-4">
                <div className="w-8 h-8 rounded-full bg-orange-700/20 text-orange-600 flex items-center justify-center font-black border border-orange-700/50">3</div>
                <div><p className="font-bold text-sm">João Lucas</p><p className="text-xs text-zinc-400">40 cotas</p></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-[#18181b] p-6 rounded-xl w-full max-w-sm border border-[#27272a] shadow-2xl relative">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#09090b] border border-[#27272a] p-4 rounded text-center">
                ...
                <p className="text-2xl font-black text-[#22c55e]">995</p>
                <p className="text-xs font-bold mt-2">Lucas M.</p>
              </div>
              <div className="bg-[#09090b] border border-[#27272a] p-4 rounded text-center">
                <p className="text-xs text-zinc-400 mb-1">Menor Título</p>
                <p className="text-2xl font-black text-red-500">002</p>
                <p className="text-xs font-bold mt-2">Fernando S.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[60]">
          <div className="bg-[#18181b] p-6 rounded-xl w-full max-w-sm border border-[#27272a] shadow-2xl relative">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-white">Caixa Premiada</h3>
              <button onClick={fecharModal} className="text-zinc-400 hover:text-white bg-[#09090b] px-3 py-1 rounded border border-[#27272a]">X</button>
            </div>
            <p className="text-sm text-zinc-400 mb-6">Configure caixas surpresas para quem comprar acima de uma certa quantidade de cotas. (Em breve)</p>
            <button onClick={fecharModal} className="w-full border border-[#27272a] text-zinc-300 py-3 rounded hover:bg-[#09090b] transition-colors">Fechar</button>
          </div>
        </div>
      )}
      ...
      {modalAtivo === 'roleta' && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[60]">
          <div className="bg-[#18181b] p-6 rounded-xl w-full max-w-sm border border-[#27272a] shadow-2xl relative">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-white">Roleta Premiada</h3>
              <button onClick={fecharModal} className="text-zinc-400 hover:text-white bg-[#09090b] px-3 py-1 rounded border border-[#27272a]">X</button>
            </div>
            <p className="text-sm text-zinc-400 mb-6">Ative a roleta para dar prêmios instantâneos ou cupons de desconto aos seus compradores. (Em breve)</p>
            <button onClick={fecharModal} className="w-full border border-[#27272a] text-zinc-300 py-3 rounded hover:bg-[#09090b] transition-colors">Fechar</button>
          </div>
        </div>
      )}
